...
export function renderStatement(s: StatementData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    if (ig) {
      ...
      y += 14;
      doc.fillColor(MUTED).font("Helvetica").fontSize(9).text(`Proctored sitting: identity photograph ${ig.identityPhoto ? "taken" : "not taken"}; ${ig.counts.photos} camera and ${ig.counts.screens} screen captures over ${ig.writingMinutes} minutes; entire screen ${ig.screenShare === "monitor" ? "shared" : "not fully shared"}; submitted by ${ig.submittedBy === "time_up" ? "the clock at time-up" : ig.submittedBy === "invigilator" ? "the invigilator" : "the learner"}.`, L, y, { width: CW });
