...
- **Restructured (5 Sep 2026)** — see `restructure-2026-09-05.md`: admin = Overview · Set up an Assessment · Register People · Schedule the Sitting · Results; FPT Exam no longer authors papers (authoring endpoints return 410 unless `ENABLE_PAPER_AUTHORING=true`); papers are uploaded (paper + memo → `ai/paperExtraction.ts`) or linked; the standard check is the gate (`intake_status`: checking/ready/blocked/override; sittings refuse anything else; admin override with audited reason); assessor dossier opens with AI marks pre-filled and tagged AI-recommended; queue badge; admin Results page. Migration 0008.
